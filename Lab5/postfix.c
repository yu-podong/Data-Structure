...
precedence getToken(char symbol)									//인자로 반든 문자를 문자형 변수로 받아 우선순위를 알려주는 함수 정의
{
...
void charCat(char* c)											//infixExp의 문자 하나를 postfixExp 뒤쪽에 연결시켜주는 함수 정의
{
	if (postfixExp == '\0')										//postfixExp가 비어있는 상태라면
		strncpy(postfixExp, c, 1);								//인자로 받은 문자를 postfixExp 뒤에 이어붇임
	else												//만약 postfixExp가 infixExp를 변환하고 있는 중이라면
		strncat(postfixExp, c, 1);								//인자로 받은 문자를 postfixExp 뒤에 이어붇임
}
...
void evaluation()											//postfixExp를 기반으로 사용자가 입력한 수식을 계산하여 결과값을 알아내는 함수 정의
{
	...
	while (*postexp != '\0')									//*postexp가 수식의 끝인 '\0'이 아니라면 즉, operand나 operator일 경우
	{
		...
		if (token == operand)									//token이 operand 즉 , 피연산자일 경우
			...
			evalStack[evalStackTop + 1] = '\0';						//evalPop함수를 통해 반환된 값은 아직 evalStack에 남아있기 때문에 pop의 구현을 위해서 반환값이 있던 위치에 '\0'를 삽입하여 원소 제거
	...
	evalResult = evalPop();										//evalPop 함수를 호출하여 evalSTack의 유일하게 저장되어 있는 수식의 결과값을 반환하고 그 값을 evalResult에 저장
}
