...
int deleteLast(headNode* h) {						//연결 리스트의 head를 이용하여 마지막 노드를 삭제하는 함수 정의
	...
	listNode *now = h->first;					//현재 위치를 나타내는 포인터를 선언하고 첫번째 노드부터 접근하기 위하 head 노드의 주소 저장
...
int invertList(headNode* h) {						//연결 리스트의 head를 이용하여 연결 리스트를 역순으로 바꾸는 함수 정의
	listNode *end, *mid = NULL, *now = h->first;			//연결 리스트를 역순으로 연결할 때 끊어지는 것을 방지하기 위해 3개의 구조체 포인터 선언 및 초기화
